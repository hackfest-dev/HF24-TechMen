...
class MagNet3Frames(object):
    ...
    def _decoder(self, texture_enc, shape_enc):
        ...
        tensor_a = texture_enc
        tensor_b = shape_enc

        # Resize tensor_b to match the shape of tensor_a along the fourth dimension
        dynamic_shape = tf.shape(tensor_a)

        # Create the size tensor using the dynamic shape
        size = tf.stack([dynamic_shape[1], dynamic_shape[2]])

        # Resize tensor_b
        tensor_b_resized = tf.image.resize(tensor_b, size)

        # Now you can concatenate tensor_a and tensor_b_resized along the fourth dimension
        enc = tf.concat([tensor_a, tensor_b_resized], axis=3)
